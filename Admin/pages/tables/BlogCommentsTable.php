...
<head>
  ...
  <title>AIFU | Blog Comments </title>
        ...
        <div class="col-12">
          ...
          <div class="card" style='overflow-x:scroll'>
            <div class="card-header">
              <h3 class="card-title">Blog Comments Records</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
               ...
               <?php
				...
				$sales6 ="SELECT comments.*,blogs.blogid,blogs.blogtitle FROM comments,blogs where blogs.blogid=comments.RepliedTo order by IDs Desc";
				$result5=mysqli_query($link,$sales6);
				$counter1=1;
				while($row5=mysqli_fetch_assoc($result5)){
                 echo"<tr>";
				 echo "<td>".$row5['Date']."</td>";
				 echo "<td style='text-align:justify;text-justify:inter-word;'>".$row5['Comment']."</td>";
                 echo "<td>".$row5['CommentBy']."</td>";
				 echo "<td>".$row5['UserEmail']."</td>";
				 echo "<td>".$row5['CommenterPhone']."</td>";
				  echo "<td>".$row5['blogtitle']."</td>";
				  $id=$row5['IDs'];
				 echo'<td><form method="get" ><a href="BlogCommentsTable.php"><button type="submit" name="delete"  value="'.$id.'" class="btn btn-block btn-danger">Delete</button></a></form></td>';  
                echo"</tr>";
				}
				?>
				<?php
				if(isset($_GET['delete'])){
					$id=$_GET['delete'];	
                        if(isset($id)){
				$sales6 ="DELETE  FROM comments WHERE IDs='".$id."'";
				$result=mysqli_query($link,$sales6);
				if($result){
				echo '<script type="application/javascript">';
				echo'alert("Comment successfully Deleted");';
				echo'window.location.href="BlogCommentsTable.php";';
				echo '</script>';
							}else{
				echo '<script type="application/javascript">';
				echo'alert("Comment Delete Failed");';
				//echo'alert("'.$id.'");';
				echo'window.location.href="BlogCommentsTable.php";';
				echo '</script>';
								}
